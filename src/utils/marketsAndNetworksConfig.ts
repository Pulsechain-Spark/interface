...
const FORK_BASE_CHAIN_ID =
  ...
  Number(global?.window?.localStorage.getItem('forkBaseChainId') || 369);
...
export const networkConfigs = Object.keys(_networkConfigs).reduce((acc, value) => {
  ...
  if (FORK_ENABLED && Number(value) === FORK_BASE_CHAIN_ID) {
    acc[FORK_CHAIN_ID] = {
      ...
      name: `${_networkConfigs[value].name}`,
      isFork: true,
      isTestnet: false,
...
export const marketsData = Object.keys(_marketsData)
  ...
  .reduce((acc, value) => {
    ...
    if (true && _marketsData[value as keyof typeof CustomMarket].chainId === FORK_BASE_CHAIN_ID) {
      acc[`fork_${value}`] = {
...
export function getNetworkConfig(chainId: ChainId | 369): NetworkConfig {
